---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

Education
======
* M.Sc. in Computer Engineering, University of Crete, 2024, GPA: 9.23/10
* B.Sc. in Informatics, Aristotle University, 2020, GPA: 7.04/10

Work experience
======
* Mar 2023 - Present: Research Fellow at Institute of Computational and Applied Mathematics (IACM)
  * Generative Models
  * Computer Vision
  * Diffusion Models

* Sep. 2021 - Sep.2023: Military Service

* Mar. 2021 - Sep. 2021: Research & Development, Ulysses Systems, Athens 
  * NLP
  * Topic Modeling
  * Learning to rank
  * Text cleansing

* Sep. 2020 - Jan. 2021: Internship, AIIA, Aristotle University of Thessaloniki
  * Recurent Neural Networks
  * Object Tracking
  
Skills
======
* Python
  * Pytorch
  * Tensorflow
  * Jax

* Latex, Wordpress

Publications
======
  <ul>
    {% for post in site.publications reversed %}
      {% include archive-single-cv.html %}
    {% endfor %}
  </ul>

Teaching
======
  <ul>
    {% for post in site.teaching reversed %}
      {% include archive-single-cv.html %}
    {% endfor %}
    <li>Teaching Assistant at the University of Crete (Sep 2022 - Present): 
      <ul>
        <li>CS472 - Computer Vision</li>
        <li>CS119 - Linear Algebra</li>
        <li>CS111 - Infinite Calculus II</li>
        <li>CS100 - Introduction to Programming</li>
      </ul>
    </li>
  </ul>

Awards
======
  <ul>
    <li>Best Paper Award, IEEE Annual Computing and Communication Workshop and Conference (CCWC), Las Vegas, NV (Jan 2019)</li>
    <li>Best Presenter Award, IEEE Annual Computing and Communication Workshop and Conference (CCWC), Las Vegas, NV (Jan 2019)</li>
  </ul>

Professional Service
======
* Military Service: Supreme Military Command of the Interior and Islands, Material Branch, Athens, Greece (Sep 2021 - Sep 2022)
* Session Chair: CP43 Topics in Dynamics III, DS19, Salt Lake City (May 2019)


  <!-- <li>Liambas, C., & Perrakis, S. “Optimization Algorithm for 3D Crater Estimation using NASA’s 2D Asteroid Data,” SIAM Conference on Applications of Dynamical Systems (DS19), Salt Lake City, UT (May 2019)</li>
    <li>Liambas, C., & Perrakis, S. “VMI Test Evaluation Optimization Algorithm,” IEEE Annual Computing and Communication Workshop and Conference (CCWC), Las Vegas, NV (Jan 2019)</li> -->